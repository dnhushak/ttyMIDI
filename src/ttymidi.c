...
// Writes to MIDI one byte at a time
void write_midi_action_from_serial_port(snd_seq_t* seq, int port_out_id, unsigned char byte,
...
void* read_midi_from_serial_port(void* seq) {
	...
	while (run) {
		...
		write_midi_action_from_serial_port(seq, port_out_id, buf, parser);
	}
}
